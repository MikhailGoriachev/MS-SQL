...
select 
    COUNT(*) as [Count] -- количество записей в выборке
from 
    books
where
    author like N'Иванов И.И.';
...
select
    MIN(books.[Public])    as minYear
    , AVG(books.[Public])  as avgYear  
    , MAX(books.[Public])  as maxYear
from
    books; 
    
-- выводим минимальную, среднюю, максимальную цену книги   
select
    MIN(books.price)    as minPrice
